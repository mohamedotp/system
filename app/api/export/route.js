    ...
    try {
        ...
        let query = `
            SELECT r.DOC_NO, 
                   ...
                   SUBSTR(LISTAGG(eg.EMP_NAME, ' | ') WITHIN GROUP (ORDER BY eg.EMP_NAME), 1, 3900) as GEHA_NAME,
                   SUBSTR(LISTAGG(eg.SEC_N, ' | ') WITHIN GROUP (ORDER BY eg.EMP_NAME), 1, 3900) as GEHA_SEC,
                   SUBSTR(LISTAGG(NVL(r.SEEN_FLAG, 0), ' | ') WITHIN GROUP (ORDER BY eg.EMP_NAME), 1, 3900) as GEHA_SEEN_FLAGS,
                   SUBSTR(LISTAGG(NVL(TO_CHAR(r.SEEN_DATE, 'YYYY-MM-DD HH24:MI'), 'N/A'), ' | ') WITHIN GROUP (ORDER BY eg.EMP_NAME), 1, 3900) as GEHA_SEEN_DATES,
