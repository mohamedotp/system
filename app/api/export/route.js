    ...
    try {
        ...
        let query = `
            SELECT r.DOC_NO, 
                   ...
                   LISTAGG(eg.EMP_NAME, ' | ') WITHIN GROUP (ORDER BY eg.EMP_NAME) as GEHA_NAME,
                   LISTAGG(eg.SEC_N, ' | ') WITHIN GROUP (ORDER BY eg.EMP_NAME) as GEHA_SEC,
                   LISTAGG(NVL(r.SEEN_FLAG, 0), ' | ') WITHIN GROUP (ORDER BY eg.EMP_NAME) as GEHA_SEEN_FLAGS,
                   LISTAGG(NVL(TO_CHAR(r.SEEN_DATE, 'YYYY-MM-DD HH24:MI'), 'N/A'), ' | ') WITHIN GROUP (ORDER BY eg.EMP_NAME) as GEHA_SEEN_DATES,
