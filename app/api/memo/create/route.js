            ...
            if (templatePath) {
                ...
                try {
                    ...
                    const psInitScript = [
                        ...
                        `# إخفاء جميع علامات التنسيق على مستوى التطبيق`,
                        `$word.Options.ShowParagraphMarks = $false`,
                        `$word.Options.ShowSpaces = $false`,
                        `$word.Options.ShowTabs = $false`,
                        `$word.Options.ShowHiddenText = $false`,
                        `$word.Options.ShowAll = $false`,
                        `try {`,
                        `    $doc = $word.Documents.Open('${escapedDestPath}', $false, $false)`,
                        `    Start-Sleep -Seconds 1`,
                        `    # إخفاء علامات التنسيق على مستوى النافذة`,
                        `    $doc.ActiveWindow.View.ShowAll = $false`,
                        `    $doc.ActiveWindow.View.ShowParagraphs = $false`,
                        `    $doc.ActiveWindow.View.ShowSpaces = $false`,
                        `    $doc.ActiveWindow.View.ShowTabs = $false`,
                        `    $doc.Fields.Update()`,
                        `    $doc.Repaginate()`,
                        `    # حفظ الملف Word بعد تعطيل علامات التنسيق`,
                        `    $doc.Save()`,
                        `    # حفظ نسخة PDF`,
