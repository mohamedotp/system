...
//             console.log("Recipients found:", resRec.rows.map(r => r[0]));

//             // Test LISTAGG
//             const sqlList = `
//                 SELECT (SELECT LISTAGG(e_other.EMP_NAME, ' | ') WITHIN GROUP (ORDER BY e_other.EMP_NAME)
//                         FROM RECIP_GEHA_NEW r_other
//                         JOIN EMP_DOC e_other ON r_other.GEHA_C = e_other.EMP_NUM
//                         WHERE r_other.DOC_NO = :docNo AND r_other.PLACE_C = :sender) as ALL_REC
//                 FROM DUAL
//             `;
//             const resList = await conn.execute(sqlList, { docNo, sender });
//             console.log("LISTAGG result:", resList.rows[0][0]);
//         }
